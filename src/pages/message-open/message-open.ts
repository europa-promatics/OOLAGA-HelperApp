...
export class MessageOpen {
  ...
  constructor(public events:Events,public http:Http,public loadingCtrl:LoadingController,private userProvider:UserProvider,public navCtrl: NavController, public navParams: NavParams) {
	...
	if(this.navParams.get('last')){
    this.sender_name=this.navParams.get('name')+' '+this.navParams.get('last').charAt(0)+'.';
	}else{
  ...
  ionViewDidLoad() {
    ...
    this.myimage="http://18.188.229.2/oolaga-french/public/img/helperfiles/" + this.userProvider.user.image;
  ...
  sendmsg(){  
    ...
    if( document.getElementById('messageBox').innerHTML != '' && document.getElementById('messageBox').innerHTML != ' '){
      ...
      let link = ENV.mainApi+'/webservicesend_message';
      let data = {
                  message:msg,
